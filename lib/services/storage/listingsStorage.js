...
import { computeSimilarityHash } from '../similarity-check/similarityCache.js';
  ...
  SqliteConnection.withTransaction((db) => {
    const stmt = db.prepare(
      `INSERT INTO listings (id, hash, provider, job_id, price, size, title, image_url, description, address,
                             link, created_at, is_active, latitude, longitude, similarity_hash)
       VALUES (@id, @hash, @provider, @job_id, @price, @size, @title, @image_url, @description, @address, @link,
               @created_at, 1, @latitude, @longitude, @similarity_hash)
    ...
    for (const item of listings) {
      ...
      const address = removeParentheses(item.address);
      const params = {
        id: nanoid(),
        hash: item.id,
        provider: providerId,
        ...
        similarity_hash: computeSimilarityHash(title, price, address),
      };
      stmt.run(params);
    }
  });
...
export const queryListings = ({
  ...
  if (freeTextFilter && String(freeTextFilter).trim().length > 0) {
    ...
    whereParts.push(
      `(title LIKE @filter OR address LIKE @filter OR provider LIKE @filter OR link LIKE @filter OR CAST(price AS TEXT) LIKE @filter)`,
    );
  }
  ...
  if (Number.isFinite(priceMin)) {
    ...
    whereParts.push('(l.price >= @priceMin)');
  }
  if (Number.isFinite(priceMax)) {
    params.priceMax = priceMax;
    whereParts.push('(l.price <= @priceMax)');
  }
  // size range filter
  if (Number.isFinite(sizeMin)) {
    params.sizeMin = sizeMin;
    whereParts.push('(l.size >= @sizeMin)');
  }
  if (Number.isFinite(sizeMax)) {
    params.sizeMax = sizeMax;
    whereParts.push('(l.size <= @sizeMax)');
  }
...
/**
 * Return listings with the same similarity_hash as the given listing,
 * excluding the listing itself. Used to show cross-provider duplicates.
 *
 * @param {string} listingId
 * @returns {{id: string, provider: string, link: string}[]}
 */
export const getSimilarListings = (listingId) => {
  return SqliteConnection.query(
    `SELECT l.id, l.provider, l.link
     FROM listings l
     WHERE l.similarity_hash = (SELECT similarity_hash FROM listings WHERE id = @listingId)
       AND l.id != @listingId
       AND l.manually_deleted = 0
     ORDER BY l.provider`,
    { listingId },
